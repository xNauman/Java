...
public class BCastTHD implements Runnable {
	...
	public void run() {
		...
		if(config.SysConfig.SYSTEM_DEBUG_MODE_ENABLED) System.out.println("Completed broadcast thread and monitoring thread start");
		...
		while(true)
		{
			...
			*/
			try{
				...
				//if(config.SysConfig.SYSTEM_DEBUG_MODE_ENABLED) System.out.println("CC's IP!!! : "+ccIPAddr);
