...
public class Clock {
	...
	{		
		//set the start time
		lSimStartTime = this.getCurrentUnixTime();
	...
	public long getSimTime()
	{
		...
		lSimCurrentTime = this.getCurrentUnixTime();
	 ...
	 *
	 */
	public void modifySimTime(int pModifier)
	{
	...
	/**
	 * 
	 * @param pOtherSimTime the unixtime in long to compair too - this would probably come from CC over RMI
	 * @return interger that represents the value the clock was out by. A + value means 
	 * our clock is behind by that amount
	 * A - value means its ahead by that amount
	 */
	public int compairTime(long pOtherSimTime)
	{
		int iModifier;
