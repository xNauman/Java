...
//    static Image  img;
    ...
    public static void main(String[] args){
      try{
         ...
         // System.out.println("Attempting to take image file from tracker");
          try{

           requestSocket = new Socket(serverAddress, socketPort);
           boolean check = server.request();
         
         if (check){
          server.sendPicViaSocket();
         in = new ObjectInputStream(requestSocket.getInputStream());
           //start Time in ms
                //byte[] buffer = server.getImage(); //using RMI
                  byte[] buffer = (byte[])in.readObject(); //using socket
           ...
           requestSocket.close(); //close socket
             File file = new File("screenshot.jpg");
             fileSize+= file.length();
           //System.out.println(file.length());

        //   System.out.println("pic of "+file.length()/1024+"kb received in "+time+" ms");
     // } //end of while
