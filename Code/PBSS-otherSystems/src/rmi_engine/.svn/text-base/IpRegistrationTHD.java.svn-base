 ...
 *
 */
public class IpRegistrationTHD implements Runnable {

	private boolean debugMode = false;
	private boolean stopIPReg = false;
	private boolean ipRegComplete = false;
	
	//ADDED FOR DUMMY MODULE SUPPORT
	private String sModName = "";
	
	public IpRegistrationTHD(String pModName){
		this.sModName = pModName;
	}
	//ADDED FOR DUMMY MODULE SUPPORT (end)
	
	public void run() {
		if(debugMode) System.out.println("Called doCCRegistration");
		//get the CCs IP
		doCCRegistration();
		
		//if(debugMode) System.out.println("Update state to "+rawobjectsPriv.EnumSysState.NOT_OPERATIONAL);
		//update state to not operational
		//utils.StateControl.setState(rawobjectsPriv.EnumSysState.NOT_OPERATIONAL, true);
		
		
		if(debugMode) System.out.println("Starting IPRegistration cc call loop");
	...
	private void doCCRegistration(){
		...
		while (!this.ipRegComplete) {
				...
				try {
					...
					if(debugMode) System.out.println("still in doCCrego");
					if (useBCast) {
						c = new cc.registry.RegistryClient(this.sModName); //specify string as CME, DB, NAV, SCH, SU, TKR
						ccIPAddr = c.getServerIP();
						if(debugMode) System.out.println("CC SERVER IS AT: " + ccIPAddr);
					}
					else {
						if(debugMode) System.out.println("************ BROADCAST NOT ACTIVE *******************");
						ccIPAddr = "127.0.0.1";
					}
					if(debugMode) System.out.println("still in doCCrego-after if");
