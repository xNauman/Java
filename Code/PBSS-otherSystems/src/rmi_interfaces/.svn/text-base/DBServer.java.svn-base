...
public interface DBServer extends Remote{
    ...
    public Bus getBus(int pBUSID) throws RemoteException,SQLException;

    public TrafficLight getTrafficLight(int pTrafficLight) throws RemoteException,SQLException;

    public BusStop getStop(int pStopID) throws RemoteException,SQLException;
    
    public Depot getDepot(int pDepotID) throws RemoteException,SQLException;


    //methods for CC
    // changed to getLogData from getLog as getLog is implemented in the Java.RemoteServer API already and hence can't be used. 
    // can't overwrite due to that being static
    // change made by: David Taberner
     ...
     * @deprecated 
     */
    boolean createSimRunEntry(int pSimRunID, int pMapToUseID) throws java.rmi.RemoteException;
	String debugEcho(String pFromModule, String pMessageToEcho) throws java.rmi.RemoteException;
	/**
     * @deprecated 
     */
	int getMaxSimRunID() throws java.rmi.RemoteException; //NOT FOR SCH:
}
