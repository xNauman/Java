...
import appControl.RMIControlTHD;
...
public class GetIPsTHD implements Runnable {
	...
	public void run() {
		...
		System.out.println("Update state to "+rawobjectsPriv.EnumSysState.NOT_OPERATIONAL);
		//update state to not operational
		util.StateControl.setState(rawobjectsPriv.EnumSysState.NOT_OPERATIONAL, true);
		
		
		System.out.println("Starting IPRegistration cc call loop");
	...
	private void doCCRegistration(){
		...
		System.out.println("Completed broadcast thread start");
	...
	private boolean getIPInfoFromCC(){
		...
		if (SysConfig.getIPregistrationCompleted()) {
			...
			boolean vErrorGettingIP_NAVIGATOR = false;
			...
			try {
				...
				rmi_interfaces.CCServer cc = new SCHClientImplCC().getSrvObject();
				...
				if (!funcGetIPInfoFromCC_Worker(EnumModName.NAVIGATOR, cc))
					vErrorGettingIP_NAVIGATOR = true;
