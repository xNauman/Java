...
public class InputTHD implements Runnable {
	...
	private void doCmdConsole(String pCmd){
		...
		else if (pCmd.equalsIgnoreCase("stop.force")||pCmd.equalsIgnoreCase("s.f")){
			this.funcPBSSForceStop();
		}
		...
		else if (pCmd.contains("debug.show")){
			this.funcPBSSdebugBusvector();
		}
		else {
	...
	private void funcPBSSdebugBusvector()
	{
		Vector vBusStore = datastore.BusMovementStore.vBusPositionVector ;
		Vector vLightStore = datastore.TrafficLightStore.vTrafficLightStore;
		long vTime = Clock.getSimTime();
		boolean vTure = true;
		vTure = false;
	}
	...
	private void funcPBSSForceStop(){
